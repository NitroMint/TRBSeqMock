...
def read_igh_fasta(fasta_path):
    ...
    with open(fasta_path, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if line.startswith('>'):
                ...
                if current_gene_name and current_seq:
                    full_seq = ''.join(current_seq).replace('.', '').replace(' ', '').replace('n', '').replace('N', '').replace('\n', '').replace('\r', '').replace('-', '').upper()
        ...
        if current_gene_name and current_seq:
            full_seq = ''.join(current_seq).replace('.', '').replace('N', '').replace('-', '').upper().replace('n', '').replace('\n', '').upper()
            sequences.append(full_seq)
            gene_names.append(current_gene_name)
            genotypes.append(current_genotype)
...
v_names, v_sequences, v_genotypes, v_full_lines = read_igh_fasta(V_FASTA)
d_names, d_sequences, d_genotypes, d_full_lines = read_igh_fasta(D_FASTA)
j_names, j_sequences, j_genotypes, j_full_lines = read_igh_fasta(J_FASTA)

def filter_vdj(full_lines, gene_names, sequences, genotypes):
    valid_indices = []
...
v_pool = filter_vdj(v_full_lines, v_names, v_sequences, v_genotypes)
j_pool = filter_vdj(j_full_lines, j_names, j_sequences, j_genotypes)
d_pool = filter_vdj(d_full_lines, d_names, d_sequences, d_genotypes)